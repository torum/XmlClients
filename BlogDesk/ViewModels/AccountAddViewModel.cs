...
public class AccountAddViewModel : ObservableRecipient, INavigationAware
{
    ...
    private string _websiteOrEndpointUrl = "https://torumyax.wordpress.com/";
    ...
    private async void OnGo()
    {
        ...
        try
        {
            ...
            // AuthRequired returned. Probably API endpoint.
            ...
            if (sr is ServiceResultHtmlPage srhp)
            {
                ...
                else
                {
                    if (srhp.HasError)
                    {
                        StatusTitleText = $"{srhp.ErrTitle}";
                        StatusText = $"{srhp.ErrDescription}";
                    }
                    else
                    {
                        StatusTitleText = "Found 0 item";
                        StatusText = "Could not find any supported services.";
    ...
    private bool CanGoSelected()
    {
        ...
        // only service document here.
        if (SelectedLinkItem is not ServiceDocumentLinkItem)
        {
            return false;
        }
